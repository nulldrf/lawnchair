...
public class Cam16 private constructor(
    ...
    public fun xyzInViewingConditions(
        ...
        val rC: Double = signum(rA) * (100.0 / viewingConditions.fl) * rCBase.pow(1.0 / 0.42)
        val gCBase: Double = max(0.0, 27.13 * abs(gA) / (400.0 - abs(gA)))
        val gC: Double = signum(gA) * (100.0 / viewingConditions.fl) * gCBase.pow(1.0 / 0.42)
        val bCBase: Double = max(0.0, 27.13 * abs(bA) / (400.0 - abs(bA)))
        val bC: Double = signum(bA) * (100.0 / viewingConditions.fl) * bCBase.pow(1.0 / 0.42)
    ...
    public companion object {
        ...
        public fun fromXyzInViewingConditions(
            ...
            val alpha: Double = (1.64 - 0.29.pow(viewingConditions.n)).pow(0.73) * t.pow(0.9)
